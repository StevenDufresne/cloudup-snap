...
public struct Uploader: Sendable {
    ...
    /// Single-step inline image upload — purpose-built for screenshots.
    /// Calls Cloudup's `upload_image` MCP tool: server-side MIME-sniffs the
    /// bytes and rejects non-image content, returning {share_url, direct_url,
    /// markdown, …}. For arbitrary non-image small files use `quick_upload`;
    /// for >2 MiB files use the begin/complete flow (see `uploadLarge`).
    public func upload(data: Data, filename: String, mime: String) async throws -> URL {
        ...
        do {
            let result = try await mcp.callTool(name: "upload_image", arguments: args)
            do {
