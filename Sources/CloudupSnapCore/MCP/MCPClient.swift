...
public actor MCPClient {
    ...
    public init(
        ...
        clientName: String = "cloudupsnap-cli",
    ...
    public func initializeIfNeeded() async throws {
        ...
        // Spec requires a `notifications/initialized` after a successful
        // initialize. Send as a proper JSON-RPC notification (no `id`) — some
        // SDKs (e.g. Cloudup's mcp/sdk) return 500 when a notification method
        // is dispatched with an id, because they try to produce a response and
        // the handler has none.
        let initialized = JSONRPCRequest.notification(method: "notifications/initialized", params: [:])
