...
- `ScreenshotterCore` (Plan 1's library) for the upload pipeline
...
**Plan 1:** `docs/superpowers/plans/2026-05-12-screenshotter-core-and-cli.md`
...
├── Package.swift                                # adds Screenshotter executable target + swift-snapshot-testing dep
├── scripts/
│   └── bundle.sh                                # wraps the binary into Screenshotter.app/Contents/{MacOS,Info.plist}
├── Sources/
│   ├── ScreenshotterCore/                       # Plan 1 — unchanged except Wallet balance helpers
│   │   └── Wallet/Wallet+Balance.swift          # NEW: balanceUSDC(...), balanceETH(...)
│   ├── Screenshotter/                           # NEW: GUI app
│   │   ├── App/
│   │   │   ├── ScreenshotterApp.swift           # @main + NSApplicationDelegate
...
│   └── screenshotter-cli/                       # Plan 1 — unchanged
└── Tests/
    └── ScreenshotterTests/                      # NEW test target for the GUI logic
...
### Task 1: Branch off and add the Screenshotter executable target
...
```
cd /Users/bongnam/dev/screenshotter
...
`Sources/Screenshotter/Placeholder.swift`:
...
```
swift build --product Screenshotter
```
...
Add Screenshotter executable target + snapshot-testing dep
...
**Files:**
- Create: `Sources/Screenshotter/App/Info.plist.template`
- Create: `scripts/bundle.sh`
- Modify: `Makefile`

- [ ] **Step 1: Write `Sources/Screenshotter/App/Info.plist.template`**
...
<dict>
  ...
  <key>CFBundleExecutable</key><string>Screenshotter</string>
  <key>CFBundleIdentifier</key><string>com.bongnam.screenshotter</string>
  <key>CFBundleName</key><string>Screenshotter</string>
  <key>CFBundleDisplayName</key><string>Screenshotter</string>
  ...
  <string>Screenshotter captures regions of your screen to annotate and share.</string>
  <key>NSAppleEventsUsageDescription</key>
  <string>Screenshotter sends notifications and posts to the clipboard.</string>
</dict>
</plist>
```
...
```
make app
ls build/Screenshotter.app/Contents/
ls build/Screenshotter.app/Contents/MacOS/
```

Expected: `Screenshotter.app/Contents/{Info.plist, MacOS/Screenshotter, Resources/...}`.
...
```
pgrep Screenshotter && echo "running" || echo "not running"
```

Kill it: `pkill Screenshotter`.

- [ ] **Step 6: Commit**

```
git add Sources/Screenshotter/App/Info.plist.template scripts/bundle.sh Makefile
...
**Files:**
- Create: `Sources/ScreenshotterCore/Wallet/Wallet+Balance.swift`
- Create: `Tests/ScreenshotterCoreTests/WalletBalanceTests.swift`
...
`Sources/ScreenshotterCore/Wallet/Wallet+Balance.swift`:
...
```
git add Sources/ScreenshotterCore/Wallet/Wallet+Balance.swift Tests/ScreenshotterCoreTests/WalletBalanceTests.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Services/ClipboardService.swift`
...
```
git add Sources/Screenshotter/Services/ClipboardService.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Services/NotificationService.swift`
...
```
git add Sources/Screenshotter/Services/NotificationService.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Annotation/AnnotationModel.swift`
- Create: `Sources/Screenshotter/Annotation/AnnotationDocument.swift`
- Create: `Tests/ScreenshotterTests/AnnotationModelTests.swift`
...
`Sources/Screenshotter/Annotation/AnnotationModel.swift`:
...
`Sources/Screenshotter/Annotation/AnnotationDocument.swift`:
...
```
git add Sources/Screenshotter/Annotation Tests/ScreenshotterTests/AnnotationModelTests.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Annotation/UndoStack.swift`
- Create: `Tests/ScreenshotterTests/UndoStackTests.swift`
...
`Sources/Screenshotter/Annotation/UndoStack.swift`:
...
```
git add Sources/Screenshotter/Annotation/UndoStack.swift Tests/ScreenshotterTests/UndoStackTests.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Render/Renderer.swift`
- Create: `Tests/ScreenshotterTests/RendererTests.swift`
...
`Sources/Screenshotter/Render/Renderer.swift`:
...
```
git add Sources/Screenshotter/Render Tests/ScreenshotterTests/RendererTests.swift Tests/ScreenshotterTests/Fixtures
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Annotation/Stickers.swift`
...
```
git add Sources/Screenshotter/Annotation/Stickers.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Capture/CaptureService.swift`
...
```
git add Sources/Screenshotter/Capture/CaptureService.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Capture/RegionSelectionOverlay.swift`
...
```
git add Sources/Screenshotter/Capture/RegionSelectionOverlay.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Annotation/ToolsPalette.swift`
...
```
git add Sources/Screenshotter/Annotation/ToolsPalette.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Annotation/AnnotationCanvas.swift`
...
```
git add Sources/Screenshotter/Annotation/AnnotationCanvas.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Annotation/AnnotationEditor.swift`
...
```
git add Sources/Screenshotter/Annotation/AnnotationEditor.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Wallet/WalletProvider.swift`
...
```
git add Sources/Screenshotter/Wallet/WalletProvider.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Hotkey/HotkeyManager.swift`
...
```
git add Sources/Screenshotter/Hotkey/HotkeyManager.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Menubar/MenubarController.swift`
...
```
git add Sources/Screenshotter/Menubar/MenubarController.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Funding/FundingPanel.swift`
...
```
git add Sources/Screenshotter/Funding/FundingPanel.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Onboarding/OnboardingCoordinator.swift`
...
```
git add Sources/Screenshotter/Onboarding/OnboardingCoordinator.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `Sources/Screenshotter/Coordination/CaptureCoordinator.swift`
...
```
git add Sources/Screenshotter/Coordination/CaptureCoordinator.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
...
### Task 21: ScreenshotterApp — @main + AppDelegate

**Files:**
- Modify (overwrite): `Sources/Screenshotter/Placeholder.swift` (rename to `ScreenshotterApp.swift`)

- [ ] **Step 1: Delete placeholder**

```
git rm Sources/Screenshotter/Placeholder.swift
```

- [ ] **Step 2: Create the real entry point**

`Sources/Screenshotter/App/ScreenshotterApp.swift`:
...
```
git add Sources/Screenshotter/App/ScreenshotterApp.swift
git -c user.email=steve.dufresne@a8c.com -c user.name="Steve Dufresne" commit -m "$(cat <<'EOF'
Add ScreenshotterApp @main with menubar + hotkey + coordinator wiring
...
- [ ] **Step 1: Grant permissions**
  - System Settings → Privacy & Security → Screen Recording → enable Screenshotter.
  - System Settings → Privacy & Security → Accessibility → enable Screenshotter.
...
This plan implements the following spec components from `docs/superpowers/specs/2026-05-12-screenshotter-design.md`:
