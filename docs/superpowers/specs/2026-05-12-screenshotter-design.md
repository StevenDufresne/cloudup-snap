...
Working name: **Screenshotter**. Trivial to rename later — the name appears only in the bundle identifier, menubar tooltip, and notification text.
...
Structured logs via `OSLog` with subsystem `com.bongnam.screenshotter`. Mirrored to `~/Library/Logs/Screenshotter/app.log` for forensic review. Every payment attempt logs: timestamp, MCP tool name, quote amount, signed tx hash (when available), result. Captures log dimensions and outcome only — never image contents.
...
- Gated behind `SCREENSHOTTER_INTEGRATION=1` + the test wallet key as a CI secret.
